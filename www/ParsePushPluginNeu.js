var serviceName = 'ParsePushPluginNeu';
      ...
      if(ParsePushPluginNeu.DEBUG){
		...
		if(pn !== null){
			if(pushAction === 'OPEN' || pn.OPEN){
				...
				//
            ParsePushPluginNeu.softTrigger(ParsePushPluginNeu._openEvent, pn);
			} else{
				//
				...
				//
				var base = ParsePushPluginNeu._receiveEvent;
				var customEventKey = ParsePushPluginNeu._customEventKey;

				ParsePushPluginNeu.softTrigger(base, pn);
				if(customEventKey && pn[customEventKey]){
					ParsePushPluginNeu.softTrigger(base + ':' + pn[customEventKey], pn);
				}
			}
		}
   };
...
//
// give ParsePushPluginNeu event handling capability so we can use it to trigger
...
var EventMixin = {
   _coldStartDelayMs: 1000,
    ...
    trigger: function(events) {
      if(this.DEBUG){
         console.log("enter ParsePushPluginNeu.trigger: " + events);
      }
      ...
      if(this.DEBUG){
         console.log("exit ParsePushPluginNeu.trigger: " + events);
      }
      return this;
   },
...
module.exports = poorManExtend(ParsePushPluginNeu, EventMixin);
